...
class ModelTrainer:
    ...
    def initiate_model_training(self) -> Path:
        ...
        try:
            # MLflow configuration belongs inside this method.
            ...
            with mlflow.start_run(
                run_name="random-forest-fraud-model"
            ) as run:
                mlflow.set_tags({
                ...
                mlflow.log_params({
                    ...
                    "trees_per_batch": self.config.trees_per_batch,
                    "max_depth": 18,
                    "min_samples_leaf": 10,
                ...
                if tree_counts[-1] != self.config.total_trees:
                    tree_counts.append(self.config.total_trees)
                ...
                self.config.model_path.parent.mkdir(
                    parents=True,
                    exist_ok=True,
                )
                self.config.metrics_path.parent.mkdir(
                    parents=True,
                    exist_ok=True,
                )
                ...
                mlflow.sklearn.log_model(model, name="fraud_model")
                ...
                self._progress(
                    f"MLflow run recorded: {run.info.run_id}"
                )
